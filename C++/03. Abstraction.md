...
- A class hierarchy offers two kinds of beneﬁts:
  ...
  - Implementation inheritance: A base class provides functions or data that simpliﬁes the implementation of derived classes.
```C++
...
```
- By default, *user-deﬁned types* and *built-in type* can be copied memberwisely.
...
- The name `this` is predeﬁned in a member function and **points to the object for which the member function is called**.
...
- **rvalue** is – to a ﬁrst approximation – a value that you can’t assign to, such as an integer returned by a function call
...
- The `template<typename T>` preﬁx makes `T` a parameter of the declaration it preﬁxes.
```C++
template<typename T>
T* begin(Vector<T>& x)
{
...
- This `sum()` is a simpliﬁed version of the standard-library `accumulate()`.
```C++
template<typename T>
class Less_than {
...
```
- A **function object** (sometimes called a functor) is used to deﬁne **objects that can be called like functions**.
...
- For a simple function object like `Less_than` inlining is simple, so that a call of `Less_than` is far more efﬁcient than an indirect function call.
...
```
- A template can be deﬁned to accept an arbitrary number of arguments of arbitrary types, which is called a *variadic template*.
...
```
- The aliasing mechanism can be used to deﬁne a new template by **binding some or all template arguments**.