...
- Type Functions
    ...
    - This technique, **called tag dispatch**, is one of several used in the standard library and elsewhere to improve ﬂexibility and performance.
    ```C++
    template<typename C>
...
- Random
  ...
  ```
  - Use of `Rand_int()` is manageable in the ﬁrst week of a C++ course for novices 
...
- Numeric Limits
  - In `<limits>`, the standard library provides classes that describe the properties of built-in types, such as the maximum exponent of a ﬂoat or the number of bytes in an int.
  ```c++
